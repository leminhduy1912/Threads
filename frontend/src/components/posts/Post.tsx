...
export default function Post({ post }: PostProps) {
  ...
  const fetchPostDetail = async () => {
    try {
      const response = await clientRequest.get(`/api/posts/${post._id}`);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // Fetch updated post detail
      await fetchPostDetail();

      // Optionally invalidate post feed cache
      queryClient.invalidateQueries(["post-feed"]);
    },
    onError: (error) => {
      console.error("Error updating post:", error);
  ...
  return (
    <article className="group/post space-y-3 rounded-2xl bg-card p-5 shadow-sm">
      <div className="flex justify-between gap-3">
        ...
        {user && user._id === currentPost.postedBy && (
          <PostMoreButton
            post={currentPost}
            className="opacity-0 transition-opacity group-hover/post:opacity-100"
          />
      ...
      <Linkify>
        <div className="whitespace-pre-line break-words">{currentPost.text}</div>
      </Linkify>

      {currentPost.img && (
        <img className="w-full h-[400px]" src={currentPost.img} alt="" />
      )}
      ...
      {showComments && <Comments post={currentPost} />}
    </article>
  );
}
...
function CommentButton({ post, onClick }: CommentButtonProps) {
  return (
    <button onClick={onClick} className="flex items-center gap-2">
      <MessageSquare className="size-5" />
      <span className="text-sm font-medium tabular-nums">
        {post.replies.length} <span className="hidden sm:inline">comment</span>
      </span>
    </button>
  );
}
