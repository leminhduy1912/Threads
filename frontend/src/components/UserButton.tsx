...
export default function UserButton({ className }: UserButtonProps) {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem("user-threads");
    console.log("Checking user from localStorage");
    try {
      const parsedUser = storedUser ? JSON.parse(storedUser) : null;
      setUser(parsedUser);
      if (!parsedUser) {
  ...
  return user ? ( // Conditional rendering based on `user` state
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className={cn("flex-none rounded-full", className)}>
          <UserAvatar avatarUrl={""} size={40} />
    ...
    <div className={cn("text-center", className)}>Loading user...</div>
  );
}
